...
import Feedback from '@/app/[lang]/(shop)/(home)/Feedback'
import { BestSeller } from '@/app/[lang]/(shop)/(home)/BestSeller'
import BestSellerSkeleton from '@/app/[lang]/(shop)/(home)/BestSellerSkeleton'
...
export default async function Home({ params }: { params: { lang: Locale } }) {
  ...
  const bestSellersProducts: Promise<BestSellerProduct[]> = axios
  ...
  return (
    <div className="flex flex-col gap-y-4 sm:gap-y-8 md:gap-y-10 xl:gap-y-12">
      ...
      <div className="mb-8 mt-8 flex flex-col items-center justify-center">
        <span className="text-2xl font-normal tracking-[0.05em]">
          ...
          <Await promise={bestSellersProducts}>
            {(bestSellersProducts) => (
              <div className="mt-6 h-[250px] w-[calc(100vw_-_24px)] max-w-[1720px] min-[500px]:h-[300px] sm:h-[350px] md:h-[350px] lg:w-[calc(100vw_-_217px)]">
                <BestSeller products={bestSellersProducts} lang={params.lang} />
              </div>
            )}
          </Await>
        </Suspense>
